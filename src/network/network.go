...
import (
	...
	"NTNUNetworkAutoLogin/src/config"
	"NTNUNetworkAutoLogin/src/logger"
	"NTNUNetworkAutoLogin/src/utils"
)
...
var (
	...
	redirectURLRegex = regexp.MustCompile(`window\.location="?(https://wificert2\.ntnu\.edu\.tw:(\d+)/fgtauth\?([^"&]+))"?;`)
...
func PerformLogin(cfg *config.Config, client *resty.Client) error {
	...
	resp, err = client.SetTimeout(defaultTimeout).
		...
		SetHeaders(map[string]string{
			"Host":       fmt.Sprintf("wificert2.ntnu.edu.tw:%s", port),
			"Origin":     fmt.Sprintf("https://wificert2.ntnu.edu.tw:%s", port),
			"Referer":    fgtauthURL,
			"User-Agent": userAgent,
		}).
		Post(fmt.Sprintf("https://wificert2.ntnu.edu.tw:%s/", port))
	...
	logoutURL := fmt.Sprintf("https://wificert2.ntnu.edu.tw:%s/logout?%s", port, match[1])
